...
const SidePanel = (props: any) => {
  ...
  return (
    <motion.div
      ...
      } ${d[1] === "r" ? "right-0" : "left-0"} p-4 bg-white shadow-lg`}
      variants={{
        open: {
          x: "0%",
        },
        ...
        {onClickSwitch && (
          <div
            className={`absolute top-[32px] ${
              d[1] === "r" ? "left-[-48px]" : "right-[-48px]"
            } w-8 h-8 inline-flex items-center justify-center rounded-${
              d[1] === "r" ? "l" : "r"
            }-lg bg-indigo-100 text-indigo-500 flex-shrink-0 cursor-pointer`}
